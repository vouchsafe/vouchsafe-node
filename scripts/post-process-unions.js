...
function createUnionType(schemaName, fileName, discriminatorProperty, mapping) {
  ...
  const imports = variants.map(([discriminatorValue, schemaRef], index) => {
    ...
    // Type name is PascalCase (as exported from the TypeScript module)
    const variantTypeName = variantSchemaName.replace(/_/g, '');
    // File path is camelCase (as generated by orval on case-sensitive Linux filesystems)
    const variantFilePath = variantTypeName.charAt(0).toLowerCase() + variantTypeName.slice(1);

    return {
      index,
      discriminatorValue,
      variantSchemaName,
      variantFileName: variantTypeName,
      variantFilePath,
      typeName: `${fileName}${index}`,
      importType: `import type { ${variantTypeName} } from './${variantFilePath}';`,
      importFromJSON: `import { ${variantTypeName}FromJSON, ${variantTypeName}ToJSON } from './${variantFilePath}';`
    };
  }).filter(v => {
    // Check if the variant file exists — use camelCase path for Linux case-sensitive filesystem compatibility
    const variantPath = path.join(modelsDir, `${v.variantFilePath}.ts`);
    if (!fs.existsSync(variantPath)) {
      console.log(`     ⚠️  Warning: Variant file not found: ${v.variantFilePath}.ts`);
      return false;
    }
    return true;
  });
...
${imports.map(v => `    case '${v.discriminatorValue}':\n      return ${v.variantFileName}FromJSON(json);`).join('\n')}
    default:
      // Unknown discriminator value — return raw JSON to stay forward-compatible with new API versions
      return json;
  }
}
...
${imports.map(v => `    case '${v.discriminatorValue}':\n      return ${v.variantFileName}ToJSON(value as ${v.variantFileName});`).join('\n')}
    default:
      // Unknown discriminator value — return raw value to stay forward-compatible with new API versions
      return value;
  }
}
`;
